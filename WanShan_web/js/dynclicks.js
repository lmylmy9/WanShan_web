...
function _showDynClicks(click, owner, clickid)
{
    try
    {
        var dynobj = "dynclicks_" + clicktype + "_" + (Math.floor(Math.random()*1000));
        document.write("<span id=\"" + dynobj + "\" name=\"" + dynobj + "\"></span>");

        var url = '/system/resource/code/news/click/dynclicks.jsp?clickid='+clickid+'&owner='+owener+'&clicktype='+clicktype;
...
function _showDynClickBatch(spanids,wbnewsids,clicktype, owner)
{
    ...
    for(var i = 0; i < spanids.length; i++)
    {
        ...
        {
            usernewsids[usernewsids.length] = spanids[i];
...
function _onShowDynClicks(xmlhttp, dynobj)
{
    if(xmlhttp.readyState == 4 && xmlhttp.status == 200)
    {
        try
        {
            ...
            if(imgobjs,length>0)
            {
                imgobjs[i].innerHTML = xmlhttp.responseText;
            }
        }
        catch(e)
        {
